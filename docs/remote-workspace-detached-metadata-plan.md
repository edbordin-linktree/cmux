...
cmux ssh <canonical-host> [--cwd <path>] [--name <title>] [--detached] [--json] [-- <cmd>]
...
The Mac Swift CLI also accepts `cmux ssh <host> --cwd <remote-path> --json -- <cmd>`. This still creates an attached Swift workspace, but the remote command no longer disables the persistent SSH PTY path: cmux creates the remote workspace, starts the normal remote shell bootstrap in the requested remote working directory, then sends `exec /bin/sh -lc <cmd>` into the returned remote terminal surface. `--detached` remains a remote-wrapper-only creation mode; using it through the Mac Swift CLI returns an explicit unsupported-mode error instead of being parsed as an unknown flag.
