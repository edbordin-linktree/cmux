...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    func testSSHPTYAttachBridgeRPCErrorDoesNotClearLocalStateBeforeBridgeAllocation() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("sshptyrpcerr")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let workspaceId = "22222222-2222-2222-2222-222222222222"
        let surfaceId = "33333333-3333-3333-3333-333333333333"
        ...
        XCTAssertTrue(result.stdout.isEmpty, result.stdout)
        XCTAssertTrue(result.stderr.contains("ssh-pty-attach: remote daemon is not ready"), result.stderr)
        let methods = state.snapshot().compactMap { self.jsonObject($0)?["method"] as? String }
        XCTAssertEqual(methods, [
            "workspace.remote.pty_bridge",
        ])
