...
nonisolated struct SessionRemoteWorkspaceSnapshot: Codable, Equatable, Sendable {
    ...
    var preferAutoConnectOnRestore: Bool? = nil
}
...
nonisolated enum SSHPTYAttachStartupCommandBuilder {
    ...
    private static func retryingAttachLines(command: String) -> [String] {
        [
            ...
            "    254|255) ;;",
            "    0)",
            "      if [ -t 2 ]; then printf '\\n\\033[33m[cmux] remote PTY attach ended. Close this terminal surface or reattach the session.\\033[0m\\n' >&2 || true; fi",
            "      while :; do sleep 3600; done",
            "      ;;",
            "    *)",
            "      if [ -t 2 ]; then printf '\\n\\033[31m[cmux] ssh-pty-attach exited with status %s.\\033[0m\\n' \"$cmux_ssh_attach_status\" >&2 || true; fi",
            "      while :; do sleep 3600; done",
            "      ;;",
            "  esac",
    ...
    private static func foregroundAuthLines(_ auth: ForegroundAuth) -> [String] {
        ...
        return [
            "cmux_ssh_auth_timeout=\"${CMUX_SSH_FOREGROUND_AUTH_TIMEOUT_SECONDS:-12}\"",
            "case \"$cmux_ssh_auth_timeout\" in ''|*[!0-9]*) cmux_ssh_auth_timeout=12 ;; esac",
            "( \(sshCommand) <&0 ) &",
            "cmux_ssh_auth_pid=$!",
            "cmux_ssh_auth_elapsed=0",
            "cmux_ssh_auth_timed_out=0",
            "while kill -0 \"$cmux_ssh_auth_pid\" 2>/dev/null; do",
            "  if [ \"$cmux_ssh_auth_elapsed\" -ge \"$cmux_ssh_auth_timeout\" ]; then",
            "    cmux_ssh_auth_timed_out=1",
            "    kill \"$cmux_ssh_auth_pid\" 2>/dev/null || true",
            "    sleep 1",
            "    kill -KILL \"$cmux_ssh_auth_pid\" 2>/dev/null || true",
            "    wait \"$cmux_ssh_auth_pid\" 2>/dev/null || true",
            "    cmux_ssh_auth_status=253",
            "    break",
            "  fi",
            "  sleep 1",
            "  cmux_ssh_auth_elapsed=$((cmux_ssh_auth_elapsed + 1))",
            "done",
            "if [ \"$cmux_ssh_auth_timed_out\" -ne 1 ]; then wait \"$cmux_ssh_auth_pid\"; cmux_ssh_auth_status=$?; fi",
            "if [ \"$cmux_ssh_auth_status\" -ne 0 ]; then",
            "  if [ \"$cmux_ssh_auth_timed_out\" -eq 1 ]; then printf '%s\\n' \"[cmux] SSH foreground authentication timed out before remote PTY attach after ${cmux_ssh_auth_timeout}s.\" >&2; else printf '%s\\n' \"[cmux] SSH foreground authentication failed before remote PTY attach (ssh exit $cmux_ssh_auth_status).\" >&2; fi",
